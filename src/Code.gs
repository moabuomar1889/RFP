  ...
  try {
    ...
    do {
      ...
      for (const project of projects) {
        ...
        // Rate limiting: sleep every 20 folders to avoid API throttling
        if (totalFolders % 20 === 0) {
          Utilities.sleep(100);
        }
      }
    ...
    // 4. Batch Write (in chunks to avoid memory issues)
    if (allRows.length > 0) {
      logRow('INFO', 'scanDriveSnapshot', `Writing ${allRows.length} rows to sheet in batches...`);
      const batchSize = 5000; // Write in chunks of 5000 rows
      for (let i = 0; i < allRows.length; i += batchSize) {
        const batch = allRows.slice(i, i + batchSize);
        const startRow = i + 2; // +2 because row 1 is header
        sheet.getRange(startRow, 1, batch.length, 8).setValues(batch);
...
function createSubfoldersRecursively(nodes, parentFolderId, pr, folderType, parentIndex) {
  ...
  nodes.forEach((node, index) => {
    // Create folder title with PR number and type
    const folderNumber = baseNumber * 100 + (index + 1); // Use hierarchical numbering
    ...
    const folderTitle = pr && folderType ? 
      `${folderNumber}-PRJ-${pr}-${folderTypeForName}-${node.text}` : 
      node.text;
...
function insertPermissionSafely(folderId, permissionBody) {
  ...
  // NOTE: Removed folder verification step here for performance
  // Verification will happen during the actual permission insert (Drive API will return error if folder doesn't exist)
  // This saves one API call per permission insert
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (attempt === 0 && (e.message.includes('rateLimitExceeded') || e.message.includes('userRateLimitExceeded') || e.message.includes('backendError'))) {
        Utilities.sleep(1500); // Wait 1.5 seconds before retry (increased from 1s)
        continue;
      }
      
      // If folder not found, skip silently (no need to retry)
      if (e.message && (e.message.includes('not found') || e.message.includes('Shared drive not found'))) {
        logRow('WARN', 'insertPermissionSafely', `Folder ${folderId} not found, skipping permission insert`);
        return false;
      }
...
function applyLimitedAccessToProject(projectId, projectTitle, templateParam) {
  try {
    // Validate inputs
    if (!projectId) {
    ...
    // Get template - use provided template if available (for performance)
    let template;
    if (templateParam && Array.isArray(templateParam)) {
      template = templateParam; // Use provided template to avoid re-fetching
    } else if (typeof projectTitle === 'string') {

/* GroupsAndAccess.gs
   Helper functions to read group list and manage members (using AdminDirectory Advanced Service).
*/

function getGroupsMap() {
  // Returns object { "GroupName": "group@domain", ... }
  ...
  try {
    ...
    // If you don't have AdminDirectory permission, return default list from Properties policy
...
function addGroupMember(groupDisplayOrEmail, memberEmail, role) {
  ...
  try {
    if (!groupEmail || !groupEmail.includes('@')) throw new Error(`Invalid group email: ${groupEmail}`);
    if (!memberEmail || !memberEmail.includes('@')) throw new Error(`Invalid user email: ${memberEmail}`);
